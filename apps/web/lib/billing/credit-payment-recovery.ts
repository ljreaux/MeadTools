...
export type CreditPaymentRecoveryResult = "applied" | "review_required" | "duplicate" | "deferred" | "ignored";
...
export async function reconcileStripeRefund(options: {
  ...
  if (!paymentIntentId || !currency || !Number.isSafeInteger(options.refund.amount) || options.refund.amount < 0) {
    return "ignored";
  }
...
async function processStoredStripeRefundEvent(eventId: string): Promise<CreditPaymentRecoveryResult> {
  return prisma.$transaction(async (tx) => {
    ...
    const adjustment = reconciliation.creditsToRevoke > 0
    ...
    const shouldRestrict = Boolean(adjustment && adjustment.availableCredits < 0);
    await tx.credit_checkout_sessions.update({
      where: { id: checkout.id },
      data: {
        refunded_amount_cents: reconciliation.refundedAmountCents,
        refunded_at: new Date(),
        status: reconciliation.refundedAmountCents === checkout.stripe_amount_total_cents
          ? "refunded"
          : "fulfilled"
      }
    });
...
export async function reconcileDeferredStripeDisputes(paymentIntentId: string): Promise<void> {
  ...
  for (const event of events) await processStoredStripeDisputeEvent(event.external_event_id);
}

async function processStoredStripeDisputeEvent(eventId: string): Promise<CreditPaymentRecoveryResult> {
  return prisma.$transaction(async (tx) => {
    ...
    const checkout = await findCheckoutForUpdate(tx, event.stripe_payment_intent_id);
    const disposition = paymentRecoveryCheckoutDisposition(checkout?.status ?? null);
    ...
    if (!existing) {
      await createReviewRecovery(tx, {
        ...
        reason: "A payment dispute requires an administrator decision before chat can resume."
      });
    }
...
export async function getCreditPaymentRecoveryAdministration() {
  ...
  return recoveries.map((recovery) => ({
    ...
    paymentRestricted: Boolean(recovery.checkout.user.credit_account?.payment_restricted_at),
    stripeDashboardUrl: recovery.recovery_kind === credit_payment_recovery_kind.stripe_dispute
...
export async function resolveCreditPaymentRecovery(options: {
  ...
  if (!Number.isSafeInteger(options.creditDelta) || Math.abs(options.creditDelta) > 1_000_000) {
    throw new RangeError("The credit adjustment must be between -1,000,000 and 1,000,000.");
  }
  const note = options.note.trim();
  if (note.length < 3 || note.length > 500) {
    throw new RangeError("Provide a resolution note between 3 and 500 characters.");
  }
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (recovery.status !== credit_payment_recovery_status.review_required) {
      throw new CreditPaymentRecoveryResolutionError("This payment recovery has already been resolved.");
    }
...
async function findCheckoutForUpdate(tx: Prisma.TransactionClient, paymentIntentId: string) {
