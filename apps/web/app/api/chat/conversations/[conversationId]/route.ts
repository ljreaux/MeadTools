...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const query = chatThreadQuerySchema.safeParse({
    beforeSequence:
      request.nextUrl.searchParams.get("beforeSequence") ?? undefined,
    limit: request.nextUrl.searchParams.get("limit") ?? undefined,
  });
  if (!path.success || !query.success) {
    return NextResponse.json(
      { error: "Invalid chat conversation request." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Unable to load chat conversation." },
      { status: 500 },
    );
  }
}
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  if (!path.success || !body.success) {
    return NextResponse.json(
      { error: "Invalid chat conversation update." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Unable to update chat conversation." },
      { status: 500 },
    );
  }
}
