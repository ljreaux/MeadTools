...
export async function PATCH(
  ...
  const parsedBody = resolveCreditPaymentRecoveryRequestBodySchema.safeParse(body);
  if (!parsedParams.success || !parsedBody.success) {
    return NextResponse.json(chatAccessErrorResponseSchema.parse({ error: "A valid recovery resolution is required." }), { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json(resolveCreditPaymentRecoveryResponseSchema.parse(result));
  } catch (error) {
    if (error instanceof CreditPaymentRecoveryNotFoundError) {
      return NextResponse.json(chatAccessErrorResponseSchema.parse({ error: error.message }), { status: 404 });
    }
    if (error instanceof CreditPaymentRecoveryResolutionError || error instanceof RangeError) {
    ...
    return NextResponse.json(chatAccessErrorResponseSchema.parse({ error: "Unable to resolve payment recovery." }), { status: 500 });
  }
}
