...
export default function CreditWallet({
  ...
  async function loadMore() {
    ...
    setLoadingMore(true);
    try {
      const page = await fetchCreditActivityPage({ token, cursor: effectiveNextCursor });
  ...
  async function startCheckout(pack: CreditPack) {
    ...
    try {
      ...
      toast({ description: t("credits.checkoutFailed"), variant: "destructive" });
      setCheckoutPackId(null);
    }
  }

  return (
    <main className={embedded ? "w-full" : "relative mx-auto mt-24 mb-24 w-11/12 max-w-4xl rounded-xl bg-background p-6 pt-16 sm:p-10 sm:pt-20"}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Button disabled={wallet.isFetching} onClick={refresh} type="button" variant="outline">
      ...
      {wallet.data?.purchasesEnabled && !paymentRestricted ? (
        <section className="mt-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">{t("credits.purchaseDescription")}</p>
            ...
            {CREDIT_PACKS.map((pack) => {
              ...
              return (
                <Button
                  ...
                  variant="outline"
                >
                  {purchasing ? <LoaderCircle className="animate-spin" /> : <Plus />}
                  {purchasing
                    ? t("credits.purchaseProcessing")
                    : t("credits.purchasePack", { credits: pack.credits.toLocaleString(), price })}
                </Button>
              );
            })}
          </div>
        ...
        {wallet.isError ? (
          ...
          <div className="mt-3 overflow-hidden rounded-md border">
            <ul className="divide-y">
              {activities.map((activity) => <CreditActivityRow activity={activity} key={activity.operationId} />)}
            </ul>
          </div>
        )}
        {effectiveNextCursor ? (
          <Button className="mt-3" disabled={loadingMore} onClick={() => void loadMore()} type="button" variant="outline">
...
function CreditActivityRow({ activity }: { activity: WalletActivity }) {
  const { t } = useTranslation();
  const label = activity.kind === "usage"
  ...
  const isDestructiveAdjustment = activity.kind === "adjustment" && activity.creditsDelta < 0;
  ...
  return (
    <li className="flex items-center justify-between gap-4 px-4 py-3 text-sm">
      ...
      <span className={cn(
        ...
        {isCredit ? "+" : "−"}{Math.abs(activity.creditsDelta).toLocaleString()}
      </span>
    </li>
  );
}
