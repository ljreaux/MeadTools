...
export const hostedAgentPolicy = {
  ...
  instructions: [
    ...
    "For a gravity-targeted mead, a named primary honey or an explicit confirmation that honey is the fermentable to adjust is a complete adjustable-fermentable choice. Call build_recipe_draft with that honey as adjustable and proceed; never ask the user to confirm the same honey choice again.",
    ...
    "Before finalizing a draft with a named additive, call search_additives once. If its catalog entry is available, use that canonical unit and standard dosage per US gallon, scaled to the requested batch volume. For an additive not in the catalog, retain a user-supplied amount only with a supported recipe-builder unit; otherwise ask for the product label or a supported amount and unit. Never invent an additive unit or dose.",
    "Recipe additive lines are stage-agnostic. If the user gives an addition time such as secondary, retain one additive line only; do not create a duplicate line for another stage or move it into Ingredients.",
