...
test("credit charges round up fractional values and apply the versioned policy", () => {
  ...
  assert.equal(
    calculateCreditCharge(1_001_000_000n, { ...feePolicy, markupBasisPoints: 0, fixedTurnCredits: 1 }),
    3
  );
...
test("a reservation blocks an insufficient account before a provider request", () => {
  assert.doesNotThrow(() => assertSufficientCredits({ availableCredits: 20, requiredCredits: 20 }));
  assert.throws(
...
test("reservation, settlement, and reversal entries keep balance derived from the ledger", () => {
  ...
  const settlement = settleReservedCredits({ reservationCredits: 12, chargedCredits: 9 });

  assert.equal(reservation, -12);
  assert.equal(settlement.settlementCreditsDelta, 3);
  assert.equal(availableCreditsFromLedger([100, reservation, settlement.settlementCreditsDelta]), 91);
...
test("effective billing versions select the newest active record", () => {
  const versions = [
    { id: "retired", effectiveAt: new Date("2026-01-01T00:00:00.000Z"), retiredAt: new Date("2026-03-01T00:00:00.000Z") },
    { id: "spring", effectiveAt: new Date("2026-03-01T00:00:00.000Z") },
    { id: "summer", effectiveAt: new Date("2026-06-01T00:00:00.000Z") }
  ];

  assert.equal(selectEffectiveVersion(versions, new Date("2026-02-01T00:00:00.000Z"))?.id, "retired");
  assert.equal(selectEffectiveVersion(versions, new Date("2026-04-01T00:00:00.000Z"))?.id, "spring");
  assert.equal(selectEffectiveVersion(versions, new Date("2026-07-01T00:00:00.000Z"))?.id, "summer");
  assert.equal(selectEffectiveVersion(versions, new Date("2025-12-01T00:00:00.000Z")), undefined);
});
