...
import type { AdditiveLineV2 } from "@/types/recipeDataV2";
...
export default function AdditivesV2() {
...
const AdditiveLineV2 = ({
  ...
  const handleAdditiveSelect = (selected: Additive) => {
    // Minimal behavior: same as V1 (only name)
    changeAdditive(selected.name);

    // Optional nice behavior (if you want):
    // changeUnit(selected.unit);
    // changeAmount(selected.dosage);
  };
